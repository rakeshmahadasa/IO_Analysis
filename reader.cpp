...
int main(int argc,char** argv){
    ...
    string in_file = "out/"+to_string(block_size)+".bin";
    FILE* ifile = fopen(in_file.c_str(),"w+b");
    vector<uint64_t> runtimes;
    for(int i=0;i<iter_count;i++){
        high_resolution_clock::time_point start_time = high_resolution_clock::now();
        fread(&block,sizeof(block),1,ifile);
        high_resolution_clock::time_point end_time = high_resolution_clock::now();
        auto time_diff = duration_cast<microseconds> (end_time - start_time).count();
